...
`public/ops-deck.html` is one self-contained file — no build step, no dependencies, no network
calls. Open it directly or serve it; both work. It is what this repository was before the Astro
site existed, and it is still published, at `/ops-deck.html`, because that is the URL that was
circulated. It is served verbatim: `checks/vercel-output.js` asserts the deployed copy is
byte-identical to the source, and that `/` is the landing page rather than the deck.

Its checks are the `deck` suite — `npm test`. See [`../checks/README.md`](../checks/README.md).

This document is the deck's own reference: how its layout decides itself, and the rules that are
easiest to undo by accident. For the **site** — publishing, the report system, the admin, the
funnel — see [`../HANDOFF.md`](../HANDOFF.md).

## Contents

- [Layout model](#layout-model--read-this-before-changing-the-frame) — read this before changing the frame
- [Other things worth knowing](#other-things-worth-knowing)
- [Tables measure their panel, not the window](#tables-measure-their-panel-not-the-window)
- [Reports has two tabs](#reports-has-two-tabs)
- [Reported figures come from the company](#reported-figures-come-from-the-company-market-data-still-does-not)
- [The view is linkable, and it survives a reload](#the-view-is-linkable-and-it-survives-a-reload)
- [Colour and type](#colour-and-type--the-rules-that-are-easiest-to-undo)
- [Verification](#verification--the-deck-suites-results) — what `npm test` holds
...
## Verification — the deck suite's results

What `npm test` holds, in detail. `run.js` serves the deck on a loopback port and hands each
check the URL in `DECK_URL`; see [`../checks/README.md`](../checks/README.md) for what each one
asserts and the two footguns that have caught people out.
